...
Success is not "the app looks impressive." Success is the user making better-informed decisions with less time, and — critically — being able to answer *"are these signals actually any good?"* from evidence rather than vibes.
...
Precise, unsentimental, fast. It reads like a working instrument, not a product demo — the voice of a terminal that respects the user's time and never oversells. Numbers are tabular and scannable; claims are hedged exactly as much as the evidence warrants and no more. It states limitations plainly ("delayed ~15 min", "not a forecast", "SIMULATED · FAKE MONEY") because credibility is the whole product. No hype, no celebration of wins, no manufactured urgency.

## Anti-references

Not a consumer fintech app (Robinhood confetti, gamified green candy, "you're crushing it" tone). Not a SaaS dashboard (hero-metric card with a gradient accent, identical rounded card grids, purple-blue gradients). Not a crypto-bro terminal (neon, glow-for-glow's-sake, moon language). Not a Bloomberg pastiche either — density is earned by information, not performed as costume.

## Design Principles

**Show the work, not the verdict.** A rating with no visible inputs is an oracle, and oracles can't be trusted or debugged. Every score decomposes; every entry states its reason and price level.

**State uncertainty at full volume.** Confidence labels, "not advice" disclosures, and the simulated banner are not legal boilerplate to be minimized — they are load-bearing UI. Real money follows this screen.
...
**One accent, spent deliberately.** Amber is the instrument's signal color and marks what matters. Green/red appear only where finance requires them (up/down). More color would mean less meaning.

**Answer "so what do I do?"** A number that doesn't lead to an action or a reason to wait is trivia. Every surface should push toward a decision or honestly say there isn't one yet.
