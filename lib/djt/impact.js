...
// Trump's register and the Federal Register's are different languages for the
// same act. "I am slapping a tariff on" and "Further Strengthening Actions
// Taken to Adjust Imports" are both impositions; only the first looks like one.
const APPLY = /\b(?:impos|rais|increas|expand|add(?:ing|ed|s)?\b|hike|introduc|slap|charg|announc|plac|putting|put a|set(?:ting)? a|will be|strengthen|tighten|restrict|safeguard|curb|further adjust|adjusting imports|adjust imports)/i;
const LIFT = /\b(?:remov|lift|cut|lower|reduc|exempt|paus|delay|drop|suspend|waiv|roll(?:ing)? back|terminat|revok|rescind|withdraw)/i;
...
export function scorePost(post, ctx = {}) {
  ...
  // --- 3. Modality (0-15)
  // A published proclamation or Federal Register filing IS the action — the
  // commitment is the document's existence, not its wording. The keyword
  // detectors are tuned to "I am imposing" and read formal legal register as
  // non-committal, which had a Section 232 aluminum proclamation scoring as
  // idle musing.
  const isInstrument = post.source === "whitehouse" || post.source === "federal_register";
  let modality = 0;
  if (isInstrument) { modality = 15; why.push("published instrument — already in force"); }
  else if (committedFlag && !hedgedFlag) { modality = 15; why.push("committed language"); }
